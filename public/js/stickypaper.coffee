...
socket.on 'created', (data) ->
  if data.slideKey === getSlideId()
    ...
    writeText = -> {
      ...
      newLabel.onmousedown = (evt) -> { onDrag(evt, this) }
      newLabel.ondblclick = (evt) -> { reEdit(evt, this) return false }
...
reEdit (evt, oDiv) -> {
  ...
  writeText = -> {
    ...
    oDiv.onmousedown = (evt) -> { onDrag(evt, this) }
    oDiv.ondblclick = (evt) -> { reEdit(evt,this) return false }
